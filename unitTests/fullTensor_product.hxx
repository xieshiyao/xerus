...
UNIT_TEST(FullTensor, Product_Threefold, 
    ...
    TEST(compare_memory_to_vector(res.data.get(), {413, 454, 937, 1030}));
    res(i,L) = B(i,J) * C(K,L) * D(J,K);
    TEST(compare_memory_to_vector(res.data.get(), {393, 458, 901, 1050}));
    res(i,L) = B(K,L) * C(i,J) * D(J,K);
    TEST(compare_memory_to_vector(res.data.get(), {477, 710, 649, 966}));
    res(i,L) = B(J,K) * C(K,L) * D(i,J);
    TEST(compare_memory_to_vector(res.data.get(), {601, 698, 725, 842}));
)
...
UNIT_TEST(FullTensor, Product_Many_Degree_2, 
    ...
    TEST(compare_memory_to_vector(res1.data.get(), {20596523, 21531582, 46728183, 48849590}));
    res1(i,o) = (A(i,j) * B(j,k)) * (C(k,l) * (D(l,m) * E(m,n)) * F(n,o)); 
    TEST(compare_memory_to_vector(res1.data.get(), {20596523, 21531582, 46728183, 48849590}));
