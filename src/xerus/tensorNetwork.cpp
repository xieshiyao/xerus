...
// 		std::sort(_measurments.begin(), _measurments.end(), SinglePointMeasurment::Comparator(degree()-1));
    ...
    void TensorNetwork::contract(const size_t _nodeId1, const size_t _nodeId2) {
        ...
        if (!node1.tensorObject) {
			...
			bool resultSparse = sparse1 && sparse2; // TODO sparse*full = full cases
			
			if (sparse1 && sparse2 && resultSparse) {
				...
				for (size_t d=0; d<node1.degree(); ++d) {
					if (node1.neighbors[d].links(_nodeId1)) {
						...
						common.emplace_back();
						lhsCurr.emplace_back(common.back());
					} else {
				...
				for (size_t d = 0; d < node2.degree(); ++d) {
					if (node2.neighbors[d].links(_nodeId2)) {
						...
						rhsCurr.emplace_back(lhsCurr[node2.neighbors[d].indexPosition]);
					} else {
				...
				std::vector<Index> resultIndices(lhsOpen);
				resultIndices.insert(resultIndices.end(), rhsOpen.begin(), rhsOpen.end());
				...
				if(!sparse1 && !sparse2 && !resultSparse) { // Full * Full => Full
					blasWrapper::matrix_matrix_product(static_cast<FullTensor*>(newTensor.get())->get_unsanitized_dense_data(), leftDim, rightDim, commonFactor, 
													static_cast<const FullTensor*>(node1.tensorObject.get())->get_unsanitized_dense_data(), trans1, midDim, 
													static_cast<const FullTensor*>(node2.tensorObject.get())->get_unsanitized_dense_data(), trans2);
				} else if(sparse1 && !sparse2 && !resultSparse) { // Sparse * Full => Full
					matrix_matrix_product(static_cast<FullTensor*>(newTensor.get())->get_unsanitized_dense_data(), leftDim, rightDim, commonFactor, 
